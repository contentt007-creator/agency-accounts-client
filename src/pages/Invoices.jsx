...
import { invoices, contacts, accounts, fmt, fmtDate } from '../api/index.js';
import { generateInvoicePDF } from '../utils/pdfGenerator.js';

const EMPTY_LINE = { description: '', quantity: 1, unitPrice: '', amount: 0 };
const EMPTY_FORM = {
  client: '', issueDate: new Date().toISOString().slice(0, 10),
...
export default function Invoices() {
  ...
  useEffect(() => {
    load();
    contacts.list({ type: 'client' }).then((r) => setClientList(r.data));
  ...
  const handleSubmit = async () => {
    if (!form.client || !form.dueDate || form.lineItems.some((l) => !l.description || !l.unitPrice)) {
    ...
    try {
      ...
      toast.error(e.response?.data?.error || 'Error creating invoice');
      ...
      <div className="card">
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {list.length === 0 && (
                <tr><td colSpan={8}><div className="empty"><div className="empty-icon">🧾</div><p>No invoices yet</p></div></td></tr>
              )}
              {list.map((inv) => (
                <tr key={inv._id} style={{ cursor: 'pointer' }} onClick={() => openDetail(inv._id)}>
                  ...
                  <td onClick={(e) => e.stopPropagation()}>
                    <div style={{ display: 'flex', gap: 4 }}>
                      {inv.status === 'draft' && (
                        <button className="btn btn-sm btn-outline" onClick={() => handleSend(inv._id)}>Send</button>
                      )}
                      {['sent', 'partial', 'overdue'].includes(inv.status) && (
                        <button className="btn btn-sm btn-success" onClick={() => openPayment(inv)}>Pay</button>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-row">
                <div className="form-group">
                  ...
                  <select value={form.client} onChange={(e) => setForm({ ...form, client: e.target.value })}>
                    ...
                    {clientList.map((c) => <option key={c._id} value={c._id}>{c.name}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label>Invoice Number</label>
                  <input placeholder="Auto-generated" value={form.invoiceNumber || ''} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                ...
                <table className="line-items">
                  ...
                  <tbody>
                    {form.lineItems.map((line, i) => (
                      <tr key={i}>
                        <td><input value={line.description} onChange={(e) => updateLine(i, 'description', e.target.value)} placeholder="Service description" /></td>
                ...
                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                  <table style={{ width: '100%', fontSize: 13 }}>
                    <tbody>
                      ...
                      {taxAmt > 0 && <tr><td className="text-muted">Tax ({form.taxRate}%)</td><td className="text-right">{fmt(taxAmt)}</td></tr>}
              ...
              <div className="form-group">
                <label>Notes</label>
                <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Payment terms, bank details, etc." />
              </div>
            </div>
      ...
      {showDetail && (
        <div className="modal-overlay" onClick={() => setShowDetail(null)}>
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body inv-preview">
              <div className="inv-header">
                <div>
                  ...
                  <div className="text-muted" style={{ fontSize: 13 }}>{showDetail.invoiceNumber}</div>
                </div>
              ...
              <div className="inv-from">
                <div className="text-muted" style={{ marginBottom: 4, fontSize: 11, textTransform: 'uppercase', fontWeight: 600 }}>Bill To</div>
                <div className="fw-600">{showDetail.client?.name}</div>
                <div className="text-muted">{showDetail.client?.email}</div>
                <div className="text-muted">{showDetail.client?.address}</div>
              </div>
              ...
              <table className="inv-totals" style={{ marginLeft: 'auto', width: 280 }}>
                <tbody>
                  ...
                  {showDetail.taxAmount > 0 && <tr><td>Tax ({showDetail.taxRate}%)</td><td className="text-right">{fmt(showDetail.taxAmount)}</td></tr>}
            ...
            <div className="modal-footer">
              {showDetail.status === 'draft' && <button className="btn btn-outline" onClick={() => handleSend(showDetail._id)}>Mark Sent</button>}
              {['sent', 'partial', 'overdue'].includes(showDetail.status) && (
                <button className="btn btn-success" onClick={() => { setShowDetail(null); openPayment(showDetail); }}>Record Payment</button>
              )}
              ...
              <button
                ...
                onClick={() => generateInvoicePDF(showDetail)}
      ...
      {showPayment && (
        <div className="modal-overlay" onClick={() => setShowPayment(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Record Payment — {showPayment.invoiceNumber}</h3>
            ...
            <div className="modal-body">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input placeholder="Bank ref, cheque #..." value={payment.reference} onChange={(e) => setPayment({ ...payment, reference: e.target.value })} />
                </div>
              </div>
              <div className="form-group">
                <label>Accounts Receivable account</label>
                <select value={payment.receivableAccountId} onChange={(e) => setPayment({ ...payment, receivableAccountId: e.target.value })}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-success" onClick={handlePayment} disabled={saving}>{saving ? 'Saving...' : 'Record Payment'}</button>
            </div>
          </div>
        </div>
      )}
