...
export default function Bills() {
  ...
  const [payment, setPayment] = useState({ amount: '', date: new Date().toISOString().slice(0, 10), reference: '', payableAccountId: '', bankAccountId: '' });
  ...
  const openPayment = (bill) => {
    setShowPayment(bill);
    setPayment({ amount: bill.amountDue, date: new Date().toISOString().slice(0, 10), reference: '', payableAccountId: '', bankAccountId: '' });
  };
  ...
  const handlePayment = async () => {
    ...
    setSaving(true);
    try {
      await bills.payment(showPayment._id, { ...payment, amount: parseFloat(payment.amount) });
